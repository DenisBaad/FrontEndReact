...
const getRelatorioFaturas = async (usuarioLogadoNome: string, dataAbertura: Date | null, dataFechamento: Date | null, status: EnumStatusFatura | null, clientesSelecionados: string[]): Promise<string> => {
  ...
  if (dataAbertura) urlRelatorio += `&dataAbertura=${dataAbertura.toISOString()}`;
  if (dataFechamento) urlRelatorio += `&dataFechamento=${dataFechamento.toISOString()}`;
